...
namespace custom
{
    ...
    bool Button(const char *label, const ImVec2 &size_arg)
    {
        ...
        state->second.colors.text = ImLerp(
            ...
            IsItemActive() || hovered ? theme::current.base.accent : theme::current.page.text,
            g.IO.DeltaTime * anim::SPEED);
    ...
    bool Checkbox(const char *label, bool *v)
    {
        ...
        state->second.colors.background = ImLerp(
            state->second.colors.background,
            *v ? theme::current.base.accent : 
            (hovered ? theme::current.elements.background_hovered : theme::current.elements.background),
            g.IO.DeltaTime * anim::SPEED);
        // 勾选标记动画
        state->second.mark_alpha = ImLerp(
            state->second.mark_alpha,
            *v ? 1.0f : 0.0f,
            g.IO.DeltaTime * anim::SPEED);
        // 文本颜色动画
        state->second.colors.text = ImLerp(
            state->second.colors.text,
            *v ? theme::current.base.accent : (hovered ? theme::current.text.hovered : theme::current.text.primary),
            g.IO.DeltaTime * anim::SPEED);
        ...
        RenderCheckMark(window->DrawList,
                        center,
                        GetColorU32(theme::current.special.checkbox_mark, state->second.mark_alpha),
                        square_sz / 2);

        // 绘制文本
        const ImVec2 text_pos = pos + ImVec2(0, (32 - CalcTextSize(label).y) / 2);

        window->DrawList->AddText(text_pos, GetColorU32(state->second.colors.text), label);
    ...
    bool CheckboxClicked(const char* label, bool* v)
    {
        ...
        GetColorU32(theme::current.text.primary), "l");
        PopFont();
    ...
    bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        && !state->second.state.hovered) state->second.opened = !state->second.opened;

        state->second.arrow_roll = ImLerp(state->second.arrow_roll,
            state->second.opened ? -1.f : 1.f, g.IO.DeltaTime * anim::SPEED);
        state->second.colors.text = ImLerp(state->second.colors.text, 
            state->second.opened ? theme::current.text.active : 
               hovered ? theme::current.text.hovered : theme::current.text.primary,
                g.IO.DeltaTime * anim::SPEED);
        state->second.colors.background = ImLerp(state->second.colors.background, 
           state->second.opened ? theme::current.elements.background_active : theme::current.elements.background,
            g.IO.DeltaTime * anim::SPEED);
        ...
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max,
             GetColorU32(state->second.colors.background), internal::rounding);

        GetWindowDrawList()->AddText(
            ImVec2(window->DC.CursorPos.x, bb.Min.y + (32 - CalcTextSize(label).y) / 2 + 1),
             GetColorU32(state->second.colors.text), label);

        PushClipRect(bb.Min, bb.Max, true);
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + ((32 - CalcTextSize(label).y) / 2),
            bb.Min.y + (32 - CalcTextSize(label).y) / 2), 
            GetColorU32(theme::current.text.active), preview_value);
        PopClipRect();

        GetWindowDrawList()->AddRectFilled(bb.Max - ImVec2(32, 32), 
                bb.Max, GetColorU32(state->second.colors.background), internal::rounding);
        GetWindowDrawList()->AddRectFilledMultiColor(
            bb.Max - ImVec2(120, 32), bb.Max,
             GetColorU32(state->second.colors.background, 0.f),
              GetColorU32(state->second.colors.background, 1.f), 
              GetColorU32(state->second.colors.background, 1.f),
                GetColorU32(state->second.colors.background, 0.f));
        ...
        GetWindowDrawList()->AddText(
            ImVec2(bb.Max.x - (32 + text_size.y) / 2,
             bb.Min.y + (32 - text_size.y) / 2),
              GetColorU32(theme::current.text.primary), "i");
        ...
        PushStyleColor(ImGuiCol_WindowBg, theme::current.elements.background.Value);
        PushStyleVar(ImGuiStyleVar_WindowRounding, internal::rounding);
        ...
        state->second.state.hovered = IsWindowHovered();

        if (multi && state->second.state.hovered && g.IO.MouseClicked[0]) state->second.opened = false;
    ...
    bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
    {
        ...
        state->second.colors.text = ImLerp(
            state->second.colors.text,
            selected ? theme::current.base.accent : theme::current.text.active,
            g.IO.DeltaTime * anim::SPEED);

        PushStyleColor(ImGuiCol_Text, GetColorU32(state->second.colors.text));
    ...
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags_InputMask_));   // Check that only 1 is selected
        ...
        if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
        {
            ...
            PushStyleColor(ImGuiCol_PopupBg, GetColorU32(theme::current.elements.background));

            PushStyleVar(ImGuiStyleVar_PopupRounding, internal::rounding);
        ...
        if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
        {
            ...
            GetWindowDrawList()->AddText(
                ...
                GetColorU32(theme::current.text.primary),  label);
    ...
    bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        state->second.colors.text = ImLerp(state->second.colors.text, 
        g.ActiveId == id ? theme::current.text.active : 
        hovered ? theme::current.text.hovered : theme::current.text.primary,
         g.IO.DeltaTime * anim::SPEED);
        ...
        GetWindowDrawList()->AddRectFilled(slider_bb.Min, slider_bb.Max,
         GetColorU32(theme::current.elements.background), 30 , ImDrawFlags_RoundCornersTop);
        ...
        state->second.slider.slow = ImLerp(state->second.slider.slow,
         grab_bb.Min.x - (frame_bb.Min.x), g.IO.DeltaTime * 25.f);

        PushClipRect(slider_bb.Min + ImVec2(1, 0), slider_bb.Max, true);

        GetWindowDrawList()->AddRectFilled(slider_bb.Min + ImVec2(0, 0), 
                ImVec2(state->second.slider.slow + (slider_bb.Min.x + 2), grab_bb.Max.y - 3), 
        GetColorU32(theme::current.base.accent), 30.f, ImDrawFlags_RoundCornersTop);
        PopClipRect();

        GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x, 
        frame_bb.Min.y + (32 - CalcTextSize(value_buf).y) / 2), 
        GetColorU32(theme::current.text.primary), value_buf);
    ...
    bool ThemeButton(const char* id_theme, bool dark, const ImVec2& size_arg)
    {
        ...
        it_anim->second.colors.background = ImLerp(it_anim->second.colors.background,
         dark || hovered ? theme::current.elements.background_hovered : theme::current.elements.background, g.IO.DeltaTime * anim::SPEED);
        ...
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max,
         GetColorU32(it_anim->second.colors.background), internal::rounding);
    ...
    void Separator_line()
    {
        GetWindowDrawList()->AddRectFilled(GetCursorScreenPos(),
         ...
         GetColorU32(theme::current.separator.separator));
        Spacing();
    }
    ...
    void SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
    {
        ...
        if (flags & ImGuiSeparatorFlags_Vertical)
        {
            ...
            window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(theme::current.elements.background));
    ...
    void Separator()
    {
        ...
        ImGuiSeparatorFlags flags = (window->DC.LayoutType == ImGuiLayoutType_Horizontal) ?
         ImGuiSeparatorFlags_Vertical : ImGuiSeparatorFlags_Horizontal;
...
bool Switch1(const char* label, bool* v)
{
    ...
    // Switch尺寸和位置计算
    const float SWITCH_WIDTH = 36.0f;  
    const float SWITCH_HEIGHT = 20.0f;
    const float CIRCLE_DIAMETER = 16.0f;
    
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImVec2(SWITCH_WIDTH, SWITCH_HEIGHT);
    ...
    // 状态管理
    static std::map<ImGuiID, internal::SwitchState> states;
    auto state = states.find(id);
    if (state == states.end())
    {
        states.insert({id, internal::SwitchState()});
        state = states.find(id);
    }
    ...
    // 动画更新
    const float target_animation = *v ? 1.0f : 0.0f;
    state->second.switch_anim = ImLerp(
        state->second.switch_anim,
        target_animation,
        g.IO.DeltaTime * anim::SPEED);
    ...
    ImU32 bg_col = GetColorU32(
        ImLerp(
            theme::current.elements.background.Value,
            theme::current.base.accent.Value,
            state->second.switch_anim
        )
    );
    ...
    draw_list->AddCircleFilled(
        ImVec2(circle_x + CIRCLE_DIAMETER * 0.5f, bb.Min.y + SWITCH_HEIGHT * 0.5f),
        CIRCLE_DIAMETER * 0.5f - 0.5f,
...
bool Switch(const char* label, bool* v)
{
    ...
    // 增大尺寸以适应更大的圆球和流体效果
    const float SWITCH_WIDTH = 40.0f;      // 增加开关宽度
    const float SWITCH_HEIGHT = 22.0f;     // 增加开关高度
    const float CIRCLE_DIAMETER = 20.0f;   // 增大圆形直径
    const float CIRCLE_SQUISH = 0.35f;     // 增加挤压系数
    ...
    if (pressed) 
    {
        *v = !(*v);
        MarkItemEdited(id);
        // 重置上一帧动画值，以产生瞬时的形变效果
        state->second.last_anim = state->second.switch_anim;
    }
    ...
    {
        // 计算动画速度
        float delta = (state->second.switch_anim - state->second.last_anim) / g.IO.DeltaTime;
        
        // 根据移动方向调整形变
        if (*v) // false -> true
        {
            squish_x = 1.0f - (delta * CIRCLE_SQUISH);
            squish_y = 1.0f + (fabsf(delta) * CIRCLE_SQUISH);
        }
        else // true -> false
        {
            squish_x = 1.0f + (delta * CIRCLE_SQUISH);
        ...
        // 限制形变范围
        squish_x = ImClamp(squish_x, 0.7f, 1.3f);
        squish_y = ImClamp(squish_y, 0.7f, 1.3f);
    }
    state->second.last_anim = state->second.switch_anim;
    ...
    // 背景
    ImVec4 bg_off = ImVec4(theme::current.elements.background);
    ImVec4 bg_on = ImVec4(theme::current.base.accent);
    ImU32 bg_col = GetColorU32(
        ImLerp(bg_off, bg_on, state->second.switch_anim)
    );
    ...
    // 圆形滑块（带形变效果）
    const float radius_x = (CIRCLE_DIAMETER * 0.5f) * squish_x;
    const float radius_y = (CIRCLE_DIAMETER * 0.5f) * squish_y;
    
    // 添加轻微阴影效果
    draw_list->AddEllipseFilled(
        ImVec2(circle_x + CIRCLE_DIAMETER * 0.5f + 1.0f, bb.Min.y + SWITCH_HEIGHT * 0.5f + 1.0f),
