...
#include <string>
    ...
    bool ChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool cap, ImGuiWindowFlags flags)
    {
        ...
        GetWindowDrawList()->AddRectFilled(parent_window->DC.CursorPos + ImVec2(0, cap ? 45 : 0), parent_window->DC.CursorPos + size_arg,
         GetColorU32(c::child::background), c::child::rounding, cap ? ImDrawFlags_RoundCornersBottom : ImDrawFlags_RoundCornersAll);

        if (cap) {

            GetWindowDrawList()->AddRectFilledMultiColor(parent_window->DC.CursorPos + ImVec2((size_arg.x / 2), 44),
                parent_window->DC.CursorPos + ImVec2((size_arg.x - 50), 45),
                GetColorU32(c::accent), GetColorU32(c::accent, 0.f),
                GetColorU32(c::accent, 0.f), GetColorU32(c::accent), c::child::rounding);
    ...
    bool Button(const char* label, const ImVec2& size_arg)
    {
        ...
        IsItemActive() || hovered ? c::page::background_active : c::page::background, g.IO.DeltaTime * 6.f);
        it_anim->second.text = ImLerp(it_anim->second.text,
         IsItemActive() || hovered ? c::accent : c::page::text,
          g.IO.DeltaTime * 6.f);
    ...
    bool Page(bool selected, const char* icon, const ImVec2& size_arg)
    {
        ...
        it_anim->second.background = ImLerp(
            ...
            selected || hovered ? c::page::background_active 
            : c::page::background, g.IO.DeltaTime * 6.f);
        it_anim->second.text = ImLerp(
            it_anim->second.text, 
            selected ? c::accent : hovered ? c::page::text_hov : c::page::text, g.IO.DeltaTime * 14.f);

        it_anim->second.alpha_line = ImLerp(it_anim->second.alpha_line,
            selected ? 1.f : 0.f, g.IO.DeltaTime * 6.f);
        it_anim->second.line_size = ImLerp(it_anim->second.line_size,
            selected ? (size_arg.x / 4) : (size_arg.x / 2), g.IO.DeltaTime * 6.f);

        it_anim->second.smooth_swap = ImLerp(it_anim->second.smooth_swap,
            selected ? 26.f : 0, g.IO.DeltaTime * 12.f);

        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max,
            GetColorU32(it_anim->second.background), c::page::rounding);

        GetWindowDrawList()->AddRectFilled(
            bb.Min + ImVec2(+it_anim->second.line_size, 
            (size_arg.y + 9)), bb.Max +
             ImVec2(-it_anim->second.line_size, 15),
              GetColorU32(c::accent, it_anim->second.alpha_line),
               c::page::rounding, ImDrawFlags_RoundCornersTop);
    ...
    bool ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
    {
        ...
        ShadeVertsLinearColorGradientKeepAlpha(draw_list, vtx_idx_0,
            vtx_idx_1, picker_pos, picker_pos + ImVec2(sv_picker_size, 0.0f), 
            col_white, hue_color32);

        draw_list->AddRectFilledMultiColor(picker_pos,
            picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), 0, 0,
            col_black, col_black, 4.f);
        ...
        for (int i = 0; i < 6; ++i)
            GetForegroundDrawList()->AddRectFilledMultiColor(
                ImVec2(picker_pos.x + i * (bars_width / 6) - (i == 5 ? 1 : 0),
                 picker_pos.y + 139),
                  ImVec2(picker_pos.x + (i + 1) * (bars_width / 6) + (i == 0 ? 1 : 0),
                   picker_pos.y + 132 + sv_bar_size - 7), 
                   col_hues[i], col_hues[i + 1], col_hues[i + 1], 
                   col_hues[i], 10.f, 
                   i == 0 ? ImDrawFlags_RoundCornersLeft : 
                   i == 5 ? ImDrawFlags_RoundCornersRight : ImDrawFlags_RoundCornersNone);
        ...
        it_anim->second.circle_move = ImLerp(
            it_anim->second.circle_move, sv_cursor_pos - bar_pos, 
            g.IO.DeltaTime * 24.f);
        ...
        if (alpha_bar)
        {
            ...
            col_black, user_col32_striped_of_alpha, user_col32_striped_of_alpha,
             col_black, 10.f);
    ...
    bool ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max, 
            GetColorU32(col_rgb), ImMin(36, 29) / 2.99f,
            ImVec2(0.f, 0.f), c::elements::rounding);
